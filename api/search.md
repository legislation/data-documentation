...
|`number`|The number of the item of legislation|Supports any [natural number](https://en.wikipedia.org/wiki/Natural_number) or `*` wildcard, as well as a natural number or `*` wildcard preceded by a [series number](../model/uris.md#legislation-numbers) code (`c`, `l`, `w`, `s`, `ni`)||
